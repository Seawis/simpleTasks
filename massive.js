...
const bqs = (obj) => {
  ...
  return arr.sort((a, b) => a.split('=')[0] - b.split('=')[0]).join('&');
};
...
/*const findWhere = (mass, obj) => mass.find((item) => Object.entries(obj)
  .every(([key, value]) => item[key] === value)) || null;*/
...
function toArabic(roman) {
  ...
  for (const key of keys) {
    if ((roman.match(new RegExp(key, 'g')) || []).length > 3) {
      return false;
    }
  }
